...
async function validateNewUser(req, res, next) {
  ...
  if (!username) {
    ...
    next({
      status: 400,
      message:
        'Password needs atleast one Uppercase Letter, one Number, and one special character'
    });
...
async function isUsernameUnique(req, res, next) {
  ...
  if (data.length) next({ status: 400, message: 'Username already exists!' });
...
module.exports = {
  createUser: [
    validateNewUser,
    asyncErrorBoundary(isUsernameUnique),
